...
#include <curvy_terrain_mapper/segmentPatch.h>
...
AnalysisResult segmentPatch::analyse_without_indices()
{
    AnalysisResult results;

    PointCloudT cloudPts = segmentCloud;
    pcl::computeMeanAndCovarianceMatrix (cloudPts, results.covariance, results.centroid);
    pcl::eigen33(results.covariance, results.eigen_vectors, results.eigen_values);


    results.coefficient[0] = results.eigen_vectors(0,0);
    results.coefficient[1] = results.eigen_vectors(1,0);
    results.coefficient[2] = results.eigen_vectors(2,0);
    results.coefficient[3] = 0;
    results.coefficient[3] = -1 * results.coefficient.dot (results.centroid);

    Eigen::Vector4f vp = Eigen::Vector4f::Zero ();
    vp -= results.centroid;
    float cos_theta = vp.dot (results.coefficient);
    if (cos_theta < 0)
    {
    ...
    // Compute the results.curvature surface change
    float eig_sum = results.covariance.coeff (0) + results.covariance.coeff (4) + results.covariance.coeff (8);
    if (eig_sum != 0)
		results.curvature = fabsf (results.eigen_values[0] / eig_sum);
    else
		results.curvature = 0;
...
void segmentPatch::getAvgNormal(Normal& result)
{
    NormalCloud cloudNms = normalCloud;
    Eigen::Vector4f normalSums(0,0,0,0);
    for(size_t pointIdx = 0; pointIdx < cloudNms.size(); pointIdx++)
    {
    	normalSums[0] += cloudNms.at(pointIdx).normal_x;
    	normalSums[1] += cloudNms.at(pointIdx).normal_y;
    	normalSums[2] += cloudNms.at(pointIdx).normal_z;
    	normalSums[3] += cloudNms.at(pointIdx).curvature;
    }
    normalSums /= cloudNms.size();
    result.normal_x = normalSums[0];
    result.normal_y = normalSums[1];
    result.normal_z = normalSums[2];
    result.curvature = normalSums[3];
}
